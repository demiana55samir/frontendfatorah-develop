...
export class SendEmailComponent implements OnInit {
  ...
  getAllClients(){
    this.http.getReq('api/admin/get/users').subscribe({ 
      // this.subs.add(this.http.getReq('api/admin/users').subscribe({ 
      next:res=>{
        this.clients=res.data;
        // this.fillClientsData(0,50)
        // this.clients =  this.clients.slice(0, 50);
        // this.processArrayInChunks(this.clients, 50);
      }
    })
  }
  ...
  getUserPlans(){
    ...
    this.subs.add(this.http.getReq(`api/admin/plans`,{params:param}).subscribe({
      next:res=>{
        ...
        // let allItem = {
        //   id:0,
        //   name: this.language == 'ar'?'الكل':'All'
        // }
        // this.PlansList.push(allItem);
    },
  }))
  }
  ...
  addEmail(){
    if(this.emailContent && this.emailContent.length > 0){
      this.emailDataForm.controls['message'].setValue(this.emailContent);
    }
    ...
    if(this.emailDataForm.valid && this.emailDataForm.dirty){
      ...
      this.subs.add(this.http.postReq('api/admin/send/emails',nonNullableValues).subscribe({
        next:res=>{
          ...
          if(this.language=='en'){
            ...
            this.toastr.info('تم أرسال البريد الألكترونى بنجاح.')
          }
        }
       }))
    }else{
